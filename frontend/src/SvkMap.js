...
export function Map({ data, handleSelect, handleNoData, ...props }) {
  ...
  const [colorPair, setColorPair] = useState(
    props.colorPair || ["#FFDFDF", "#430500"]
  );
  const [colorPalette, setColorPalette] = useState(
    chroma
      .scale([colorPair[0], colorPair[1]])
      .mode("hsl")
      .colors(legendBuckets)
      .map((item, index) => ({ color: item, limit: index * legendInterval }))
      .reverse()
  );
  const [regions, setRegions] = useState(
    map.svg.g.path.map((item, index) => ({
      id: item["-id"].replace("district", ""),
      name: item["-id"],
      d: item["-d"],
      selected: false,
      color: defaultColor,
  ...
  function handleColorPalette(n_colors, interval, colorPair) {
    setColorPalette(
      chroma
        ...
        .mode("hsl")
        .colors(n_colors)
        .map((item, index) => ({ color: item, limit: index * interval }))
        .reverse()
    );
  }
  ...
  useEffect(() => {
    ...
    if (select) {
      ...
      handleSelect({
        id,
        name: okresy[id].title,
        value: data ? data[id] : "Missing",
        selected: select,
      });
    } else {
      handleSelect(select);
  ...
  useEffect(() => {
    if (Object.keys(data).length !== 0) {
      setRegions(
        regions.map((item, id) => {
          const regionValue = parseFloat(data[item.id]?.primary.value);
          return {
            ...item,
    ...
    <>
      <div className={classes.root}>
        {position.x && (
          ...
          >
            <CardContent>
              <Typography>{position.region.title}</Typography>
              <Divider className={classes.dividerMargin} />
              <Typography className={classes.bold}>
                {position.data
                  ? `${position.data.primary.name}: ${
                      position.data.primary.value ||
                      position.data.primary.value === 0
                        ? position.data.primary.value
                        : "Not available"
                    }`
                  : "Data not available"}
              ...
              {position.data?.secondary &&
                Object.entries(position.data?.secondary).map(
                  ([key, value], index) => {
                    return (
        ...
        <svg width="100%" viewBox="0 0 290.36 144.18">
          <g>
                    ...
                    >
                      <Grid item>
                        <ColorPicker
                          ...
                          hideTextfield
                          onChange={(color) =>
                            setColorPair([`#${color.hex}`, colorPair[1]])
                          }
                        />
                      </Grid>
                      <Grid item>
                        <ColorPicker
                          ...
                          hideTextfield
                          onChange={(color) =>
                            setColorPair([colorPair[0], `#${color.hex}`])
                          }
                        />
                      </Grid>
                    </Grid>
