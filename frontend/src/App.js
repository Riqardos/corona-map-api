...
export const myAxios = axios.create({
  ...
  baseURL:
    window.env?.API_URL ||
    "http://localhost:8080/https://data.korona.gov.sk/api",
});
...
function App() {
  ...
  const getData = (url, primaryName, primaryValue, secondary) => {
      ...
      .then((res) => {
        let result = res.data.page
          ...
          .reduce((acc, item, index) => {
            ...
            acc[item.district_id] = {
              primary: { name: primaryName, value: item[primaryValue] },
              secondary: second,
            };
            return acc;
          }, {});
  ...
  useEffect(() => {
    switch (tabIndex) {
      case 0:
        ...
        getData("/ag-tests/by-district", "Positivity rate", "positivity_rate", {
  ...
  useEffect(() => {
    ...
    getData("/ag-tests/by-district", "positivity_rate", {
      positives_count: "Number of positive tests",
      negatives_count: "Number of negative tests",
      positives_sum: "Sum of positive tests",
      negatives_sum: "Sum of negative tests",
  ...
  return (
    <>
      ...
      <Snackbar
        open={!dataAvailability}
        autoHideDuration={6000}
        onClose={() => setDataAvailability(true)}
